...
import {
  createProjectReplyAction,
  editProjectPostAction,
  deleteProjectPostAction,
  flagProjectPostAction,
} from '../actions'
import {
  muteUserAction,
  unmuteUserAction,
...
export default function ProjectPostItem({
  post,
  projectId,
  currentUserId,
  isMutedByMe,
  ...
  return (
    ...
    >
      {isHeld && (
        ...
        >
          <span className="font-medium">Held for review.</span> The automatic
          filter matched this post.
      ...
      {mode === 'editing' ? (
        <form action={editProjectPostAction} className="mt-3 flex flex-col gap-2">
        ...
        <div className="mt-3 flex flex-wrap items-center gap-x-4 gap-y-2 text-sm">
          ...
          {isAuthor && (
            <>
              ...
              <form action={deleteProjectPostAction} className="inline">
                <input type="hidden" name="post_id" value={post.id} />
                <input type="hidden" name="project_id" value={projectId} />
                <button
                  type="submit"
            ...
            <>
              ...
              <form action={blockUserAction} className="inline">
                ...
                <button
                  ...
                  className="text-stone-500 hover:text-stone-800 hover:underline"
                  onClick={(e) => {
                    if (!confirm(`Block ${post.author_username}?`))
                      e.preventDefault()
                  }}
                >
                  Block
          ...
          {isSignedIn && !isAuthor && (
            <form action={flagProjectPostAction} className="inline">
              <input type="hidden" name="post_id" value={post.id} />
              <input type="hidden" name="project_id" value={projectId} />
